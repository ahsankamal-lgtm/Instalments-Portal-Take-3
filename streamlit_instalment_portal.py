...
# -----------------------------
def landing_page():
    """Renders a visually appealing landing page with a Call-to-Action."""
    
    # 1. Page Setup & Styling
    st.markdown("""
    <style>
    /* Remove padding and margin for the main content block */
    .block-container {
        padding-top: 1rem;
        padding-bottom: 0rem;
        padding-left: 2rem;
    ...
    with col_hero_1:
        st.markdown(
            "<h1>Drive Green. Pay Easy. ⚡</h1>"
            "<p style='font-size: 24px; color: #31333F;'>Instant pre-qualification for electric vehicle installment plans. Fast, fair, and focused on sustainability.</p>"
            , unsafe_allow_html=True
        )

        # Call-to-Action (CTA) Button - directs user to the main form
        if st.button("Start Your Application Now 🚀", type="primary", use_container_width=True):
            st.session_state.current_tab_index = 0 # Direct to the 'Applicant Information' tab
            st.experimental_rerun() # Rerun the app to switch tabs

        st.markdown("<p style='font-size: 14px; margin-top: 10px;'>Secure your future-ready ride in under 5 minutes.</p>", unsafe_allow_html=True)

    with col_hero_2:
        # Placeholder for an attractive image/video of an EV bike
        # NOTE: Using an external image URL here for demonstration
        st.image("https://images.unsplash.com/photo-1627915576059-e37d05711685?q=80&w=1974&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")

    st.markdown("<br><br><hr>", unsafe_allow_html=True) # Separator

    # 3. Features / Trust Bar
    st.subheader("Why Choose EV Finance?")
    
    col_f1, col_f2, col_f3 = st.columns(3)
    
    with col_f1:
        st.metric(label="Quick Decision", value="75%", delta="Score for Approval")
        st.write("Our data-driven model provides instant scoring and decisions.")
    with col_f2:
        st.metric(label="Eco-Friendly", value="Zero Emissions", delta="Future Proof")
        st.write("Financing only electric bikes, supporting a cleaner environment.")
    with col_f3:
        st.metric(label="Flexible Terms", value="Low EMI", delta="Competitive Rates")
        st.write("Custom installment plans designed for affordability.")
...
# Session state initialization for Navigation (Landing Page = -1, Tabs start at 0)
if 'current_tab_index' not in st.session_state:
    st.session_state.current_tab_index = -1 
if 'applicant_valid' not in st.session_state:
    st.session_state.applicant_valid = False

# Variables used in the evaluation section must be initialized to avoid errors when switching from landing page
# We'll initialize them only if they don't exist in session state
if 'net_salary' not in st.session_state:
    st.session_state.net_salary = 0
    st.session_state.emi = 0
    st.session_state.applicant_bank_balance = 0
    st.session_state.guarantor_bank_balance = 0
    st.session_state.salary_consistency = 0
    st.session_state.employer_type = "Govt"
    st.session_state.job_years = 0
    st.session_state.age = 18
    st.session_state.dependents = 0
    st.session_state.residence = "Owned"
    st.session_state.bike_type = "EV-1"
    st.session_state.bike_price = 0
    st.session_state.outstanding = 0
    ...
    st.session_state.area_address = ""
    st.session_state.city = ""
    st.session_state.state_province = ""
    st.session_state.postal_code = ""
    st.session_state.country = ""
    st.session_state.guarantors = "No"
    st.session_state.female_guarantor = None
    st.session_state.electricity_bill = "No"
...
if st.session_state.current_tab_index == -1:
    ...
    # Use the session state variables to hold data across tabs/reruns
    first_name = st.session_state.first_name
    last_name = st.session_state.last_name
    cnic = st.session_state.cnic
    phone_number = st.session_state.phone_number
    gender = st.session_state.gender
    guarantors = st.session_state.guarantors
    female_guarantor = st.session_state.female_guarantor
    electricity_bill = st.session_state.electricity_bill
    education = st.session_state.education
    occupation = st.session_state.occupation
    street_address = st.session_state.street_address
    area_address = st.session_state.area_address
    city = st.session_state.city
    state_province = st.session_state.state_province
    postal_code = st.session_state.postal_code
    country = st.session_state.country
    net_salary = st.session_state.net_salary
    emi = st.session_state.emi
    applicant_bank_balance = st.session_state.applicant_bank_balance
    guarantor_bank_balance = st.session_state.guarantor_bank_balance
    salary_consistency = st.session_state.salary_consistency
    employer_type = st.session_state.employer_type
    job_years = st.session_state.job_years
    age = st.session_state.age
    dependents = st.session_state.dependents
    residence = st.session_state.residence
    bike_type = st.session_state.bike_type
    bike_price = st.session_state.bike_price
    outstanding = st.session_state.outstanding
    
    
    # Define tabs based on the main application structure
